        ...
        CookieData?.role == "recruiter" ? (
            ...
            <Dialog>
              <div className="w-[70%] h-[75%] bg-secondary p-5 text-sm flex flex-col gap-3">
                <div className="flex gap-2 items-center font-semibold">
                  <IndianRupee size={30} className="bg-background p-2 rounded-md" />{PostData?.SalaryRange || "N/A"}
                </div>

                <div className="flex gap-2 items-center font-semibold">
                  <MapPin size={30} className="bg-background p-2 rounded-md" />{PostData?.JobType || "N/A"}
                </div>

                <li className="self-center">{PostData?.Location}</li>

                <div className="flex gap-2 items-center font-semibold">
                  <CalendarSearch size={30} className="bg-background p-2 rounded-md" /> {PostData?.ExperienceLevel || "N/A"}
                </div>

                <DialogTrigger asChild><Button onClick={FetchApplicants}>Applicants</Button></DialogTrigger>

                <div className="flex gap-2 items-center font-semibold"><CircleAlert size={17} />
                  <p className="text-xs text-neutral-500 text-center">Last Date {PostData?.ApplicationDeadline}</p>
                </div>
              </div>
              <DialogContent>
                <DialogHeader>
          ...
          <div className="w-full h-full row-span-2 flex justify-center items-center">
            <Dialog>
              ...
              <DialogContent>
                <DialogHeader>
                  <DialogTitle>{`Apply to ${PostData?.CompanyName}`}</DialogTitle>
                  <DialogDescription>
                    <>
                      <Textarea
